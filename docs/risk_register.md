...
| R-DATA-046 | Research | Exploratory intraday results are mistaken for cross-day validation. | Medium | High | Persist campaign roles in configs, registries, manifests, status reports, and docs; require exploratory labels until multi-day completion. | Open | 3B.1+ | Research |
| R-ENG-022 | Engineering | Runtime migration after collection makes attempts incomparable. | Low | High | Allow `CAMPAIGN_RUNTIME_MIGRATED` only before any collection attempt, with unchanged config/policy hashes and append-only ledger evidence. | Open | 3B.1+ | Engineering |
| R-ENG-023 | Engineering | Multiple campaigns contaminate each other's registry, lock, or status. | Medium | High | Partition campaign state by validated campaign ID and test two campaigns in one registry root. | Open | 3B.1+ | Engineering |
| R-ENG-024 | Engineering | Mocked campaign tests bypass real Phase 2D runtime preflight. | Medium | High | Add integration-level offline tests from campaign slot execution into paired collection using mocked collectors only. | Open | 3B.2+ | Engineering |
| R-DATA-047 | Operations | Zero-data preflight failures consume scheduled campaign slots. | Medium | Medium | Preserve failed slots in the ledger, migrate only when no evidence was collected, and use later or reserve slots. | Open | 3B.2+ | Data |
| R-DATA-048 | Operations | Campaign message caps silently fall back to quality-policy defaults and truncate long slots. | Medium | High | Use explicit `None` fallback semantics, share the `100,000` max across config/runtime, and test the full campaign-to-collector path. | Mitigated by Phase 3B.3; monitor first corrected live slot. | 3B.3+ | Data |
| R-ENG-025 | Engineering | Corrective runtime migration after rejected evidence obscures historical attempt lineage. | Medium | High | Preserve excluded attempts, append `CAMPAIGN_RUNTIME_MIGRATED_AFTER_EXCLUDED_ATTEMPTS`, record source hashes, and validate future attempt runtime lineage. | Open | 3B.3+ | Engineering |
| R-DATA-049 | Operations | Collector diagnostic sinks terminate archival campaign collection before requested run limits. | Medium | High | Use a discard sink for archival production collection, keep explicit `RunLimits` as the stopping contract, and record per-venue effective limits and stop reasons. | Mitigated by Phase 3B.4; monitor first corrected live slot. | 3B.4+ | Data |
